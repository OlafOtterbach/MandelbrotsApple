...
import { debounceTime, filter, bufferTime, map, sampleTime } from 'rxjs/operators';
...
@Component({
  ...
  imports: [FormsModule, MatSliderModule, MatInputModule],
})
export class MandelbrotViewComponent implements AfterViewInit {
  ...
  public async onResizeSlow(_: Event) {
    ...
    if (canvasWidth >= canvasHeight) {
      this.imageSize = new ImageSize(
        this.imageBase,
        Math.floor(this.imageBase / aspectRatio)
      );
    } else {
      this.imageSize = new ImageSize(
        Math.floor(this.imageBase * aspectRatio),
  ...
  private getImageData(imageSize: ImageSize): ImageData {
    return this.context.getImageData(0, 0, imageSize.Width, imageSize.Height);
  }
}
